...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message{
    ...
    if (self.contactsDelegate && [self.contactsDelegate respondsToSelector:@selector(didReceiveMessage)]) {
        [self.contactsDelegate didReceiveMessage];
    }
}
//已发送消息
- (void)xmppStream:(XMPPStream *)sender didSendMessage:(XMPPMessage *)message{
    NSLog(@"已成功发送消息：%@",message);
    if (self.contactsDelegate && [self.contactsDelegate respondsToSelector:@selector(messageDidSend)]) {
        [self.contactsDelegate messageDidSend];
    }
}
#pragma mark 刷新消息的方法
-(void)reloadMessage{
    ...
    //创建谓词，设置过滤条件
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"streamBareJidStr = %@ AND bareJidStr = %@", [YSMXMPPManager shareManager].myJid.bare,self.contactsJid.bare];
